.getInsertsPos <- function(atacFrag, motifData, stranded, shiftLeft, scoreCol=NULL){
  ...
  if(stranded){
    specColsFrag <- c("sample", "strand", scoreCol)}
  else{
    specColsFrag <- c("sample", scoreCol)}
...
#' @param peakRanges [GenomicRanges::GRanges-class] object containing coordinates of peak-matches.
#' @param motifRanges [GenomicRanges::GRanges-class] object containing coordinates of motif-matches. Needs to contain a `motif_id` metadata column.
#' If `NULL` the `getpmoi` function will be used
...
#' @param weightCol If rather a weight for the fragments should be used. If `NULL` all fragments will be counted with a weight of one.
#' @param BPPARAM Parallel back-end to be used. Passed to [BiocParallel::bpmapply()].
#' @param ... Passed to `getpmoi`.
...
getInsertionProfiles <- function(atacData,
  ...
  if(!("sample" %in% colnames(atacFrag[[1]]))){
    ...
    sampleLevels <- unique(atacFrag[["sample"]])
  }
  ...
  if(calcProfile){
    ...
    if(sum(atacProfiles$w, na.rm=TRUE)==0){
      # in case of very low coverage
      atacProfiles[,w:=pos_count_global, by=motif_id]
    }
    ...
    atacProfiles[,w:=w/sum(w, na.rm=TRUE), by=motif_id]
  }
  else{
  ...
  motifScores <- BiocParallel::bpmapply(function(md,af,
    ...
    atacInserts <- .getInsertsPos(af, md, stranded, shiftLeft, scoreCol=scoreCol)
    ...
    if(!is.null(profiles)){
      if(is.null(scoreCol)){
        ...
        atacInserts <- atacInserts[,.(pos_count=sum(get(scoreCol))),
                                   by=.(motif_match_id, motif_id,
                                        sample, rel_pos, type)]
      }
      ...
      if(is.null(scoreCol)){
        ...
        atacInsertSum <- atacInserts[,.(pos_count=sum(get(scoreCol))),
                                     by=.(motif_match_id, motif_id, sample, type)]
      }
    }
  ...
  motifScores[,sample:=factor(sample, levels=sampleLevels)]

  if(simplified){
    scoreCol <- fifelse(!is.null(atacProfiles), wInsertsFeatName, insertFeatName)
    assayMats <- .genomicRangesMapping(peakRanges,
                                       motifScores,
                                       scoreCol=scoreCol,
                                       byCols=c("motif_id", "sample"),
                                       aggregationFun=sum)

    res <- SummarizedExperiment(rowRanges=peakRanges,
                                assays=assayMats,
